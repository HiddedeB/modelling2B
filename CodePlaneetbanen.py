...
def args(smaxis1, smaxis2, m_1, m_2, m_c):
    ...
    integrabel_function_1 = lambda phi: np.cos(phi) / (1 - 2 * alpha12 * np.cos(phi) + alpha12 ** 2) ** (
                3 / 2)  # angle phi
    integrabel_function_2 = lambda phi: np.cos(2 * phi) / (1 - 2 * alpha12 * np.cos(2 * phi) + alpha12 ** 2) ** (
                3 / 2)  # angle 2*phi
...
def ODE_solv(smaxis1, smaxis2, m_1, m_2, m_c, e1, e2, omega1, omega2, I1, I2, Omega1, Omega2, time, t_ev):
    ...
    return solve_ivp(sys_of_eq, [0, time], IC, args=args(smaxis1, smaxis2, m_1, m_2, m_c), t_eval=t_ev,
                     dense_output=True)
...
for i in range(np.size(z[0])):
    ...
    plot_elips_3d(theta_values, original_system_mercury[0], m.smaxis, original_system_mercury[3],
                  original_system_mercury[2], original_system_mercury[1], 'r')
    plot_elips_3d(theta_values, original_system_venus[0], v.smaxis, original_system_venus[3], original_system_venus[2],
                  original_system_venus[1], 'grey')
    ax.plot(0, 0, 0, 'ko')
